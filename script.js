...
document.getElementById("sendBtn").addEventListener("click", function (e) {
  ...
  if (!name || !email || !message) {
    formStatus.innerHTML =
      '<span class="error-icon">&#10071;</span> Please fill out all fields.';
    formStatus.style.color = "red";
  } else if (!emailPattern.test(email)) {
    formStatus.innerHTML =
      '<span class="error-icon">&#10071;</span> Invalid email format.';
    formStatus.style.color = "red";
  } else {
    ...
    emailjs
      .send("service_1d7izpm", "template_ai29wdu", {
        name: name,
        email: email,
        message: message,
      })
      .then(function () {
        formStatus.innerHTML =
          '<span class="success-icon">&#9989;</span> Message sent successfully!';
        formStatus.style.color = "green";
      ...
      .catch(function (error) {
        formStatus.innerHTML =
          '<span class="error-icon">&#10071;</span> Failed to send the message.';
        formStatus.style.color = "red";
        console.error(error);
